...
namespace OtpTrayApp
{
    ...
    public class ResourceMonitor : IDisposable
    {
        ...
        private string GenerateHtmlTemplate(string jsonData, MonitoringSession session)
        {
            ...
            html.AppendLine("        .loading { text-align: center; color: #4ec9b0; padding: 50px; }");
            html.AppendLine("        .error { background: #f48771; color: #000; padding: 15px; border-radius: 5px; margin: 20px 0; }");
            html.AppendLine("    </style>");
            html.AppendLine("</head>");
